	...
	implements Mage_Payment_Model_Recurring_Profile_MethodInterface
{
	...
	{
		// Handle min/max check for RPs
		if( $quote->hasRecurringItems() ) {
			if ($checksBitMask & self::CHECK_ORDER_TOTAL_MIN_MAX) {
				// We can only get price from the item. (??) So let's find the item.
				$nominalItem	= null;
				...
				if( $nominalItem instanceof Mage_Sales_Model_Quote_Item ) {
					$total = $nominalItem->getBaseRowTotalInclTax();
			...
			// Skip parent min/max check (we just did it)
