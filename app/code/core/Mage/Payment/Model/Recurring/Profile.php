...
class Mage_Payment_Model_Recurring_Profile extends Mage_Core_Model_Abstract
{
    ...
    {
		Mage::log('isValid', null, 'KITDEBUG_STARTDATE.log', true);
        $this->_filterValues();
        $this->_errors = array();
    ...
    {
		Mage::log('importBuyRequest Fired', null, 'KITDEBUG_STARTDATE.log', true);
        $startDate = $buyRequest->getData(self::BUY_REQUEST_START_DATETIME);
		Mage::log($startDate, null, 'KITDEBUG_STARTDATE.log', true);
        if ($startDate) {
    ...
    {
		// Log for debug
		Mage::log('setNearestStartDatetime', null, 'KITDEBUG_STARTDATE.log', true);
		Mage::log($minAllowed, null, 'KITDEBUG_STARTDATE.log', true);
		...
		// If session date is set, override date.
        if(Mage::getSingleton('core/session')->getRecurringProfileStartDateFlag()){
			$date = new Zend_Date(Mage::getSingleton('core/session')->getRecurringProfileStartDateFlag());
		}

		// Log for debug
		Mage::log('Recurring Profile Session Variable :', null, 'KITDEBUG_STARTDATE.log', true);
		Mage::log(Mage::getSingleton('core/session')->getRecurringProfileStartDateFlag(), null, 'KITDEBUG_STARTDATE.log', true);
    ...
    {
		Mage::log('_filterValues', null, 'KITDEBUG_STARTDATE.log', true);
    ...
    {
		Mage::log('_validateBeforeSave Fired', null, 'KITDEBUG_STARTDATE.log', true);
        if (!$this->isValid()) {
    ...
    {
		Mage::log('_beforeSave', null, 'KITDEBUG_STARTDATE.log', true);
