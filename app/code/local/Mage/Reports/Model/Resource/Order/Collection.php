...
class Mage_Reports_Model_Resource_Order_Collection extends Mage_Sales_Model_Resource_Order_Collection
{
    ...
    protected function _prepareSummaryLive($range, $customStart, $customEnd, $isFilter = 0)
    {
        ...
        $this->getSelect()
			...
			//->where('main_table.status = '.Mage_Sales_Model_Order::COMPLETE)
    ...
    public function calculateSales($isFilter = 0)
    {
		...
		$statuses = Mage::getSingleton('sales/config')
            ->getOrderStatusesForState(Mage_Sales_Model_Order::STATE_COMPLETE);
        ...
        if (Mage::getStoreConfig('sales/dashboard/use_aggregated_data')) {
            ...
            $this->getSelect()->where('main_table.order_status =(?)', $statuses);
        } else {
            $this->setMainTable('sales/order');
            $this->removeAllFieldsFromSelect();
            ...
            $this->getSelect()
                ...
                ->where('main_table.status =(?)', $statuses)
                ->where('main_table.state NOT IN(?)', array(
                    Mage_Sales_Model_Order::STATE_NEW,
                    Mage_Sales_Model_Order::STATE_PENDING_PAYMENT)
                );
    ...
    {
        $this->addFieldToFilter('status', array('eq' => Mage_Sales_Model_Order::STATE_COMPLETE));
        $this->getSelect()
