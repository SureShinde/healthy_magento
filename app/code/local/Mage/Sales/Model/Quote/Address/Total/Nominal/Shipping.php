...
class Mage_Sales_Model_Quote_Address_Total_Nominal_Shipping extends Mage_Sales_Model_Quote_Address_Total_Shipping
{
    ...
    public function collect(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        if (!count($items)) {
			...
			// Escaping the function to avoid shipping costs on recurring profiles
			return $this;
		}
        ...
        foreach ($items as $item) {
            if (!$item->getProduct()->isVirtual()) {
                ...
                if ($baseAmount) {
                    $item->setShippingAmount($address->getQuote()->getStore()->convertPrice($baseAmount, false)); // NEXT IT - I ATTEMPTED TO FIX SHIPPING PRICE ISSUE WITH THIS LINE BUT REVERTED BECAUSE IT DID NOT FIX MY ISSUE
                }
            }
        }
