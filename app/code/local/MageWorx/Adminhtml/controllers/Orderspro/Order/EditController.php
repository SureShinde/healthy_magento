...
class MageWorx_Adminhtml_Orderspro_Order_EditController extends MageWorx_Adminhtml_Orderspro_Order_Edit_Abstract {
    ...
    public function saveAction() {
        try {
            ...
            if (!$customer->getId()) {
                $customer->setSharedStoreIds(array(Mage::getSingleton('adminhtml/session_quote')->getStoreId()))->setId(true);
            }
...
//        }
        ...
        foreach($prevItems as $prevItem) {
            $itemsData[$prevItem->getProductId()]['item_id'] = $prevItem->getItemId();
            $itemsData[$prevItem->getProductId()]['parent_item_id'] = $prevItem->getParentItemId();
        }        
        ...
        foreach ($itemsData as $productId=>$data) {
            $select = $write->select()->from($coreResource->getTableName('sales_flat_order_item'), array('item_id'))->where('`order_id` = '.$orderPrevId . ' AND `product_id` = '. $productId);
