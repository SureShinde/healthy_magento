...
class Interapptive_ShipWorks_Model_ObjectModel_Api extends Mage_Api_Model_Resource_Abstract
{
	...
	function WriteOrderItems($orderItems, $storeCode)
	{
		...
		foreach ($orderItems as $item)
		{
            ...
            {
                $this->writeStartTag("Attribute");
                $this->writeElement("Name", "Kit_Information");
                ...
                $additionalOptions = strip_tags(str_replace(array("<i>", "</i>"), array("_", "_"), $additionalOptions));

                $this->writeElement("Value", $additionalOptions);
                $this->writeCloseTag("Attribute");
            }
	...
	function ExecuteOrderCommand($order, $command, $comments, $carrierData, $tracking, $sendEmail = false)
	{
		try
		{
	...
	function updateOrder($order, $command, $comments, $tracking, $carrierData, $sendEmail)
	{
		...
		$this->ExecuteOrderCommand($order, $command, $comments, $carrierData, $tracking, $sendEmail);
