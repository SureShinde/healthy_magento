...
class PAJ_BuyXGetYFree_Frontend_Checkout_CartController extends Mage_Checkout_CartController
{
	...
	public function couponXgetYfreeCartUpdate($productYID,$couponRequired,$productYDesc,$cartTotalRequired)
    {
        ...
        $productYCartItemId = null;
		static $lowStockWarningAmount = 5;
		if ($cartCouponCode == 'recurring50'){
            foreach ($cart->getQuote()->getAllItems() as $item) {
                $itemProduct = $item->getProduct();
                $productId = $itemProduct->getId();
                $itemOptions = $itemProduct->getTypeInstance(true)->getOrderOptions($itemProduct);
                ...
                if(isset($periodType) && $periodType != '-1'){
                    // This makes sure the discount isn’t applied over and over when refreshing
                    $originalProduct = Mage::getModel('catalog/product')->load($productId);
		...
		}else if( $cartCouponCode == 'recurring150'){
			foreach ($cart->getQuote()->getAllItems() as $item) {
        		$itemProduct = $item->getProduct();
        		$productId = $itemProduct->getId();
                $itemOptions = $itemProduct->getTypeInstance(true)->getOrderOptions($itemProduct);
                ...
                if(isset($periodType) && $periodType != '-1'){
                    ...
                    // Grab the "Original Price" - or theprice
