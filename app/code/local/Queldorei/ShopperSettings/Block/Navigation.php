...
class Queldorei_ShopperSettings_Block_Navigation extends Mage_Catalog_Block_Navigation
{
	                                               ...
	                                               $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
	{
		...
		// Adding code to add active class (6/19/2013-AV)
		$xCategory = $this->getCategoryUrl($category); // Grab the current category URL
		$xCount = substr_count($xCategory, '/'); // Count the number of slashes in our string
		// Perform the removal for sub-nav only
		if($xCount == 4) {
			$xSub = basename($xCategory); // Grab the end of the category URL
			$xCategory = rtrim(str_replace($xSub, '', $xCategory), '/'); // Remove the base directory and trailing slash
		}
		$xActive = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]"; // Grab the current page URL
		$xCount = substr_count($xActive, '/'); // Count the number of slashes in our string
		// Perform the removal for sub-nav only
		if($xCount == 4) {
			$xSub = basename($xActive); // Grab the end of the category URL
			$xActive = rtrim(str_replace($xSub, '', $xActive), '/'); // Remove the base directory and trailing slash
		}
		// Check if the two are the same
		if($xCategory == $xActive) {
			// Create active class
			$xClass = 'hhm_active';
		}
		// Otherwise, no class
